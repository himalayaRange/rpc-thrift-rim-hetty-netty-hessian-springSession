...
/**
 * 
 * ClassName: HettyConfig  
 * Function: Hetty配置
   <br>
 * @author wangyi
   <br>
 * @date: 2017-2-13 上午11:15:42 
   <br> 
 * @version  
   <br>
 * @since JDK 1.7
...
public class HettyConfig {
	...
	private static final Logger logger = LoggerFactory.getLogger(HettyConfig.class);
	...
	public void loadPropertyFile(String file){
		if(StringUtils.isEmpty(file)){
			throw new IllegalArgumentException("Paramter of file can not be blank");
		}
	...
	/**
	 * get the service properties file,default is hetty-config.xml
	 * @return
	 */
	public String getpropertiesFile() {
		String f = properties.getProperty("properties.file", "hetty-config.xml");
		return f.trim();
	}
		...
		try {
			...
			if (list.size() == 0) {
				Class<?> defaultClazz = classLoader
						.loadClass("com.github.wangyi.hetty.plugin.XmlConfigPlugin"); //依据实际的项目路径进行修改
